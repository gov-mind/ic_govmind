...
echo "Checking if ic_govmind_proposal_analyzer canister is deployed..."
if ! dfx canister id ic_govmind_proposal_analyzer > /dev/null 2>&1; then
    echo "Error: ic_govmind_proposal_analyzer canister is not deployed"
    echo "Please deploy first: dfx deploy ic_govmind_proposal_analyzer"
    exit 1
fi

# Set the API key
echo "Setting API key..."
dfx canister call ic_govmind_proposal_analyzer set_api_key "(\"$DEEPSEEK_API_KEY\")"
