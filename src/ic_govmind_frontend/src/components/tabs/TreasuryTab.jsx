...
import { ArrowDownRight, ArrowUpRight, BarChart3, AlertTriangle, Send } from 'lucide-react';
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
      ...
      tooltip: {
        ...
        callbacks: {
          label: function (context) {
            ...
            return `${asset.chain}: ${asset.percentage}% ($${(asset.usdValue || 0).toLocaleString()})`;
          },
        },
      },
    },
...
function AITreasuryAnalysis({ data }) {
  const assets = data?.assets || [];
  const totalBalance = data?.totalBalance || { usd: 0, change: { percentage: 0 } };

  const calculateHealthScore = () => {
    let score = 10;
    const maxAllocation = assets.length ? Math.max(...assets.map(a => a.percentage || 0)) : 0;
  ...
  const maxAsset = assets.reduce((max, a) => (a.percentage || 0) > (max.percentage || 0) ? a : max, assets[0] || { chain: 'N/A', percentage: 0 });
  const growthPercentage = totalBalance.change?.percentage || 0;

  const getHealthColor = (score) => {
    const s = Number(score);
    if (s >= 8) return { bg: 'bg-emerald-50', border: 'border-emerald-200', text: 'text-emerald-700', badge: 'bg-emerald-100 text-emerald-700' };
    if (s >= 6) return { bg: 'bg-amber-50', border: 'border-amber-200', text: 'text-amber-700', badge: 'bg-amber-100 text-amber-700' };
  ...
  return (
    <div className={`${healthColors.bg} ${healthColors.border} border rounded-xl overflow-hidden`}> 
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <span className={`text-xs px-3 py-1 ${healthColors.badge} rounded-full font-medium`}>
              {Number(healthScore) >= 8 ? 'Excellent' : Number(healthScore) >= 6 ? 'Good' : 'Needs Attention'}
            </span>
          </div>
        </div>
      </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-600 leading-relaxed">
                  Treasury grew by <span className="font-medium text-green-600">+{growthPercentage}%</span> this period, driven by strong {maxAsset.chain} performance.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/70 backdrop-blur-sm rounded-lg p-4 border border-white/50">
            <div className="flex items-start space-x-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-600 leading-relaxed">
                  Staking idle assets could yield <span className="font-medium text-purple-600">~${((totalBalance.usd || 0) * 0.06).toLocaleString()}</span> annually.
                </p>
              </div>
            </div>
          </div>
...
export default function TreasuryTab({
  ...
  const txs = (transactions && transactions.length) ? transactions : (data.transactions || []);
  ...
  const tokenDefs = [
    { key: 'icp', symbol: 'ICP', tokenName: 'ICP', chain: 'Internet Computer', decimals: decimalsMap.ICP },
    { key: 'dao', symbol: dao?.base_token?.symbol || 'DAO', tokenName: dao?.base_token?.name || 'DAO', chain: 'Internet Computer', decimals: decimalsMap.DAO },
    { key: 'btc', symbol: 'BTC', tokenName: 'BTC', chain: 'Bitcoin', decimals: decimalsMap.BTC },
    { key: 'eth', symbol: 'ETH', tokenName: 'ETH', chain: 'Ethereum', decimals: decimalsMap.ETH },
    { key: 'usdt', symbol: 'USDT', tokenName: 'USDT', chain: 'Ethereum', decimals: decimalsMap.USDT },
  ];

  const liveBalanceItems = tokenDefs.map(td => {
    const nat = tokenBalances?.[td.key] ?? 0n;
    const amount = typeof nat === 'bigint' ? scaleByDecimals(nat, td.decimals) : Number(nat || 0);
...
const orderedChains = [
  ...preferredChainOrder.filter((c) => groupedByChain[c] && groupedByChain[c].length > 0),
  ...
  const submitTransfer = async () => {
    ...
    try {
      ...
      if (!recipient || recipient.trim().length === 0) throw new Error('Recipient address is required');
      ...
      if (typeof selectedAsset.baseUnits === 'bigint' && amountUnits > selectedAsset.baseUnits) {
      ...
      if (selectedAsset.chain === 'Internet Computer') {
        ...
        wallet_address = (daoWalletAddresses?.icrc1?.owner && typeof daoWalletAddresses.icrc1.owner.toText === 'function')
      ...
      if (!wallet_address) throw new Error('DAO wallet address not available for this chain');
         ...
         <div className="bg-white border border-slate-200 rounded-xl p-6">
           {tvl !== undefined ? (
             <>
               ...
               <div className="text-2xl font-bold text-slate-900">${Number(tvl || 0).toLocaleString()}</div>
             </>
           ) : (
             <>
               <div className="text-slate-500 text-sm">Total Treasury</div>
               <div className="text-2xl font-bold text-slate-900">${Number(totalBalanceUsd).toLocaleString()}</div>
               <div className={`mt-2 inline-flex items-center text-sm font-medium ${changeDir === 'up' ? 'text-green-600' : 'text-red-600'}`}>
                 {changeDir === 'up' ? (
                   <ArrowDownRight className="w-4 h-4 mr-1 rotate-180" />
                 ) : (
                   <ArrowUpRight className="w-4 h-4 mr-1" />
                 )}
                 {changeDir === 'up' ? '+' : ''}{changePct}%
               </div>
             </>
           )}
         </div>
           ...
           {inflow24h !== undefined ? (
             <>
               ...
               <div className="flex items-center space-x-2">
                 ...
                 <div className="text-2xl font-bold text-slate-900">{Number(inflow24h || 0).toLocaleString()} {symbol}</div>
               </div>
             </>
           ) : (
             <>
               <div className="text-slate-500 text-sm">Assets</div>
               <div className="text-2xl font-bold text-slate-900">{stats.totalAssets ?? assets.length}</div>
             </>
           )}
         </div>
         <div className="bg-white border border-slate-200 rounded-xl p-6">
           {outflow24h !== undefined ? (
             <>
               ...
               <div className="flex items-center space-x-2">
                 ...
                 <div className="text-2xl font-bold text-slate-900">{Number(outflow24h || 0).toLocaleString()} {symbol}</div>
               </div>
             </>
           ) : (
             <>
               <div className="text-slate-500 text-sm">Active Chains</div>
               <div className="text-2xl font-bold text-slate-900">{stats.activeChains ?? new Set(assets.map(a => a.chain)).size}</div>
             </>
           )}
         </div>
         <div className="bg-white border border-slate-200 rounded-xl p-6">
           {tokenPrice !== undefined ? (
             <>
               ...
               <div className="text-2xl font-bold text-slate-900">${Number(tokenPrice || 0).toFixed(2)}</div>
             </>
           ) : (
             <>
               ...
               <div className="flex items-center space-x-2">
                 <ArrowDownRight className="w-4 h-4 text-green-500 rotate-180" />
                 <div className="text-2xl font-bold text-slate-900">{(stats.monthlyGrowth ?? changePct).toLocaleString()}%</div>
               </div>
             </>
           )}
         </div>
         ...
         <div className="space-y-4">
           <div className="bg-white border border-slate-200 rounded-xl p-6">
             <div className="flex items-center justify-between mb-4">
               <h3 className="text-lg font-semibold text-slate-900">Treasury Balances</h3>
             </div>
             {isBalancesLoading ? (
               ...
               <div className="text-center py-6">
                 ...
                 <p className="text-slate-700 text-sm">Failed to load balances.</p>
               </div>
             ) : (
               <div className="space-y-4">
                   ...
                   <div key={chain}>
                     ...
                     <div className="space-y-2">
                       {groupedByChain[chain].map((item) => (
                         <div key={`${chain}-${item.symbol}`} className="flex items-center justify-between">
                           <div className="flex items-center space-x-3">
           ...
           <div className="mt-4 space-y-2">
             {assets.map((asset, idx) => (
               <div key={idx} className="flex items-center justify-between text-sm">
                 <div className="flex items-center space-x-2">
                   <span className="inline-block w-2.5 h-2.5 rounded-full" style={{ backgroundColor: asset.color || '#64748b' }} />
                   <span className="text-slate-700">{asset.chain} ({asset.symbol})</span>
                 </div>
             ...
             <tbody className="divide-y divide-slate-200">
               {assets.map((a, idx) => (
                 <tr key={idx} className="text-sm text-slate-700">
                   ...
                   <td className="py-2">${(a.usdValue ?? 0).toLocaleString()}</td>
                   <td className="py-2">
                     <div className="w-32 bg-slate-100 h-2 rounded-full overflow-hidden">
                       <div className="h-full bg-blue-500" style={{ width: `${a.percentage || 0}%` }} />
                     </div>
                   </td>
                 </tr>
               ))}
         ...
         <div className="p-6">
           {transactionsLoading ? (
             ...
             <div className="text-center py-12">
               ...
               <h3 className="text-lg font-medium text-slate-900 mb-2">Error Loading Transactions</h3>
               <p className="text-slate-600">Failed to fetch treasury transactions.</p>
             </div>
           ) : txs && txs.length > 0 ? (
                 ...
                 <tbody className="divide-y divide-slate-200">
                   {txs.map((tx, idx) => (
                     <tr key={idx} className="text-sm text-slate-700">
                       ...
                       <td className="py-2">{(tx.amount ?? 0).toLocaleString()}</td>
                       <td className="py-2">${(tx.usdValue ?? 0).toLocaleString()}</td>
                       <td className="py-2">{tx.time || (tx.timestamp ? new Date(Number(tx.timestamp) / 1_000_000).toLocaleString() : '-')}</td>
                       <td className="py-2 font-mono">{tx.hash ? `${tx.hash.slice(0, 8)}...${tx.hash.slice(-8)}` : (tx.other_party ? `${tx.other_party.slice(0, 8)}...${tx.other_party.slice(-8)}` : '-')}</td>
                     </tr>
                   ))}
                 </tbody>
               </table>
         ...
         <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
           <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl sm:max-w-3xl border border-slate-200">
             ...
             <div className="p-5 space-y-4">
               <div>
                 <label className="block text-sm text-slate-600 mb-1">Recipient Address</label>
                 <input
                   type="text"
                   value={recipient}
               ...
               <div>
                 <label className="block text-sm text-slate-600 mb-1">Amount ({selectedAsset.symbol})</label>
                 <input
                   type="text"
                   inputMode="decimal"
                   ...
                   className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                 />
                 <p className="text-xs text-slate-500 mt-1">Decimals: {selectedAsset.decimals}</p>
               </div>
               ...
               <div className="flex items-center justify-end space-x-3 pt-2">
                 <button onClick={closeTransfer} className="px-3 py-2 text-slate-600 hover:text-slate-800">Cancel</button>
                 <button
                   onClick={submitTransfer}
                   disabled={transferLoading}
                   className={`${transferLoading ? 'bg-blue-300' : 'bg-blue-600 hover:bg-blue-700'} px-4 py-2 rounded-lg text-white`}
                 >
